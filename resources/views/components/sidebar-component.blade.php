            ...
            @foreach ($elements as $element)
                @if (Auth::user()->role && Auth::user()->role->name == $element['role'])
                    @php
                        ...
                        // Comprobar si la ruta actual coincide
                        $selected = request()->is(ltrim($route, '/') . '*') ? 'bg-blue-500 text-white' : '';
                    @endphp

                    <div onclick="window.location.href = '{{ $route }}'"
                        class="flex items-center gap-3 text-sm rounded-md px-3 py-2 group cursor-pointer hover:bg-blue-500 hover:text-white transition-all {{ $selected }}">
                        <span
                            class="material-symbols-outlined text-blue-500 group-hover:text-white transition-all {{ $selected ? 'text-white' : '' }}">{{ $element['icon'] }}</span>
                        {{ $element['label'] }}
                    </div>
                @endif
            @endforeach
