...
class ShoppingCart extends Component
{
    ...
    public function updateAmount($id, $value)
    {
        $item = CartProduct::find($id);
        $amount = intval($value);

        if ($item && $amount > 0) {
            // Si es producto normal, comprobar stock
            if ($item->product_id && $amount <= $item->product->stock) {
                $item->amount = $amount;
                $item->save();
            }
            // Si es producto mayorista, comprobar stock
            elseif ($item->wholesaler_product_id && $amount <= $item->wholesalerProduct->stock) {
                $item->amount = $amount;
                $item->save();
            }
            // Si no hay suficiente stock
            else {
                $this->dispatch('toastr', ['type' => 'error', 'message' => 'No hay suficiente stock disponible']);
            }
        }
        $this->refreshItems();
    }
    ...
    public function render()
    {
        ...
        $this->wholesalers = Wholesaler::whereIn(
            'id',
